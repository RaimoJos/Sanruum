...
class User(Base):
    ...
    health_record = relationship(
        'HealthRecord', back_populates='user',
        cascade='all, delete-orphan',
    )
    diseases = relationship(
        'Disease', secondary=user_diseases,
        back_populates='users', lazy='subquery',
    )
    user_medications = relationship(
        'UserMedication', back_populates='user',
        cascade='all, delete-orphan',
    )
    health_media = relationship(
        'HealthMedia', back_populates='user',
        cascade='all, delete-orphan',
    )
    ...
    lab_tests = relationship(
        'LabTest', back_populates='user',
        cascade='all, delete-orphan',
    )
    doctor_visits = relationship(
        'DoctorVisit', back_populates='user',
        cascade='all, delete-orphan',
    )
    mood_logs = relationship(
        'MoodLog', back_populates='user',
        cascade='all, delete-orphan',
    )
